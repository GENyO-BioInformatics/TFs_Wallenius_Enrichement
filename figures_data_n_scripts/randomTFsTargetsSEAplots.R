...
iqrCols <- colnames(fullContent)[grep("IQR",colnames(fullContent))]
iqrMeltedDF <- melt(fullContent, id.vars=idCols, measure.vars=iqrCols)
colnames(iqrMeltedDF) <- c(idCols,"typeOfAnalysis","RankingIQR")
iqrMeltedDF$typeOfAnalysis <- gsub("ranking_|_IQR","",iqrMeltedDF$typeOfAnalysis)

fullContentMelted <- merge(merge(permutMeltedDF,rankMedianMeltedDF),iqrMeltedDF)
