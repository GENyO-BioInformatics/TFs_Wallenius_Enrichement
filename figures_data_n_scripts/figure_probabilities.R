...
# cargar collectri
collectri <- read.delim("data/collectri.tsv")
...
ann_prob <- lapply(annotations, function(ann){
  ...
  anntable <- read.delim(glue("data/{anname}.tsv")) %>% filter(organism == 9606)
...
write.csv(ann_prob_tbl, file = "data/probabilities.csv", row.names = F, quote = F)

################################################################################
### 2. PLOT DATA
################################################################################

ann_prob_tbl <- read.delim("data/probabilities.csv",sep = ",")
ann_prob <- split(ann_prob_tbl, f = ann_prob_tbl$ann)
...
ggsave("figures_data_n_scripts/figure_probabilities.tiff", plot = p,units = "cm",height = 6, width = 8,dpi = 500, compression = "lzw", bg = "white")

ggsave("figures_data_n_scripts/figure_probabilities.png", plot = p,units = "cm",height = 6, width = 8,dpi = 500, bg = "white")
